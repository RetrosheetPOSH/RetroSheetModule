Clear-Host;
$script:ModuleName = 'RetroSheetModule'; # The name of your PowerShell module
$script:ProjectName = "RetroSheetModule"; # The name of your C# Project
$script:DotnetVersion = "net7.0"; # The version of .Net the project is targeted to
$script:GithubOrg = 'RetrosheetPOSH' # This could be your github username if you're not working in a Github Org
...
$script:Source = $PSScriptRoot; # This will be the root of your Module Project, not the Repository Root
...
$script:Assemblies = Join-Path $script:Destination 'assemblies'; # Supporting assemblies will be stored in this folder
...
Write-Host -ForegroundColor Green "Assemblies    : $($script:Assemblies)";
...
Task Clean -depends CleanProject {
 $null = Remove-Item $script:Output -Recurse -Force -ErrorAction Ignore
 $null = New-Item -Type Directory -Path $script:Destination
 $null = New-Item -Type Directory -Path $script:Assemblies
}
...
Task CleanProject -Description "Clean the project before building" -Action {
 dotnet clean "$($script:Source)\$($script:ProjectName).sln"
}

Task BuildProject -Description "Build the project" -Action {
 dotnet build "$($script:Source)\$($script:ProjectName).sln" -c Release
}

Task GetRetroSheet -Description "Download the Retrosheet Models" -Action {
 nuget install Retrosheet -o $script:Assemblies 
}

Task BuildManifest -Description "Compile the Module Manifest" -depends GetRetroSheet -Action {
 $Functions = @()
 $PackageName = (Get-Item "$($script:Assemblies)").GetDirectories().BaseName;

 write-Host $PackageVersion

 $RequiredAssemblies = @()
 $RequiredAssemblies += "assemblies\$($PackageName)\lib\$($script:DotnetVersion)\RetroSheet.dll"
 Update-Metadata -Path $script:ManifestPath -PropertyName RequiredAssemblies -Value $RequiredAssemblies
}
...
Task ReleaseNotes -Description "Create release notes file for module manifest" -Action {
 $Github = (Get-Content -Path "$($script:Source)\github.json") | ConvertFrom-Json
 $Credential = New-Credential -Username ignoreme -Password $Github.Token
 Set-GitHubAuthentication -Credential $Credential
 $Milestone = (Get-GitHubMilestone -OwnerName $script:GithubOrg -RepositoryName $script:ModuleName -State Closed | Sort-Object -Property ClosedAt)
 if ($Milestone)
 {
